...
export const AmbulanciaStats: React.FC<AmbulanciaStatsProps> = ({ ambulancia }) => {
  const stats = [
    { 
      ...
      value: ambulancia.length, 
      icon: Truck, 
      color: 'bg-blue-100 text-blue-600' 
    },
    { 
      label: 'Disponibles',  
      value: ambulancia.filter(a => a.estado.estado === 'Disponible').length, 
      icon: AlertCircle, 
      color: 'bg-green-100 text-green-600' 
    },
    { 
      label: 'En Servicio', 
      value: ambulancia.filter(a => a.estado.estado === 'En servicio').length, 
      icon: Clock, 
      color: 'bg-yellow-100 text-yellow-600' 
    },
    { 
      label: 'En Mantenimiento', 
      value: ambulancia.filter(a => a.estado.estado === 'En mantenimiento').length, 
      icon: Activity, 
