...
import { useRouter } from "next/router" // Importación corregida para Pages Router
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
     // const response = await fetch("http://localhost:3001/auth/login", {
        ...
        const response = await fetch("https://backendamed-production.up.railway.app/auth/login", {
        method: "POST",
        headers: {
      ...
      if (userRole === "SuperAdmin" || userRole === "Admin") {
        ...
        // Manejar roles inesperados o redirección por defecto
        setError("Rol de usuario desconocido. Contacte al administrador.")
        // Opcionalmente, redirigir a una página genérica o cerrar sesión
        ...
        <div className="relative bg-white bg-opacity-50 backdrop-blur-lg rounded-3xl shadow-2xl p-8 w-full max-w-md border border-teal-200">
          ...
          <div className="relative z-10">
            <div className="flex flex-col items-center mb-8">
              ...
              <p className="text-teal-600 text-center"> Sistema Integral De Gestion De Traslados y Ambulancias</p>
            </div>
